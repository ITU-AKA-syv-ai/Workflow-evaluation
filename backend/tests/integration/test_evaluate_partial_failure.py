...
def test_evaluate_partial_failure_rule_based_and_llm_judge(
    ...
    response = client_with_registry.post("/evaluations", json=_make_partial_failure_request(), headers=headers)
...
def test_evaluate_partial_failure_excludes_failed_evaluator_weight(
    ...
    response = client_with_registry.post(
        "/evaluations", json=_make_partial_failure_request(llm_judge_weight=5), headers=headers
    )
