...
def test_insert_works_correctly(db_session):
    repo = SQLAlchemyResultRepository(db_session)
    initial_count = db_session.query(Result).count()  # Check if the table is empty

    repo.insert(AggregatedResultEntity(request="test", result="test"))
...
def test_insert_with_multiple_rows_should_have_unique_id(db_session):
    repo = SQLAlchemyResultRepository(db_session)
    ...
    all_ids = {r.id for r in all_results}  # creates a set of all ids, which cannot have duplicates
