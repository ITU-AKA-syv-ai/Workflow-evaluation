...
export default function EvaluationDetails() {
  ...
  const total_execution_time = useMemo(() => {
    ...
    return data.result.results.reduce(
  ...
  return (
    <div className="page-container">
      ...
      <div className="summary-card">
        <p>
          <strong>Evaluation ID:</strong> {data.id}
        </p>
      </div>
      <br />
      <div className="summary-container">
        <div className="summary-card">
          ...
          {_created_at.toLocaleString()}
        </div>
        <div className="summary-card">
          <strong>SCORE: </strong>
          {data.result ? data.result.weighted_average_score.toFixed(2) : "N/A"}
        </div>
        <div className="summary-card">
          <strong>FAIlURES: </strong>
          {data.result ? `${data.result.failure_count} errors` : "N/A"}
        </div>
        <div className="summary-card">
        ...
        <div className="card-container">
          {data.request.configs.map((r, i) => (
            <div className="card" key={i}>
              <p>
                <strong>Evaluator:</strong> {r.evaluator_id}
              </p>
              <p>
              ...
              {r.threshold !== null && (
                <p>
                  <strong>Pass threshold:</strong> {r.threshold}
                </p>
              )}
          ...
          <div className="card-container">
            {data.result.results.map((r, i) => (
              <div className="card" key={i}>
                ...
                <div
                  className={`status-badge ${r.passed ? "passed" : "failed"}`}
                >
                  {r.passed ? "Passed" : "Failed"}
                </div>
                <p>
                  <strong>Score:</strong> {r.normalised_score.toFixed(2)}
                </p>
                <p>
