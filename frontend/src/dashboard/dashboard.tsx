...
function Chart({ data, evaluators }: ChartProps) {
  ...
  const labels = filteredData.map((a) =>
    dateToFormattedString(a.created_at as Date),
  );
  ...
  const datasets = [aggregatedDataSet].concat(
    Object.entries(dataPerEvaluator)
      .map(([key, scores]) => {
  ...
  if (datasets.length == 1) {
    return <div>No data found within the given dates.</div>;
  }
...
function EvaluatorGraph({ data, evaluators }: ChartProps) {
  const distributionTabs = evaluators.map((evaluator) => {
    return {
      label: evaluator.evaluator_id,
      component: <Distribution data={data} evaluator={evaluator} />,
    };
  });
...
function Filters({ setStartDate, setEndDate }: FiltersProps) {
  ...
  return (
    <div className="filters">
      <div
        style={{ display: "flex", gap: "2em", padding: "1em 1em 0.5em 1em" }}
      >
        <DateTimePicker
          label="Start date"
